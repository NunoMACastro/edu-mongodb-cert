...
A forma geral separa três documentos com responsabilidades diferentes:

```javascript
updateOne(filter, update, options);
```

- `filter` escolhe o estado que pode ser alterado;
- `update` descreve a transformação e deve usar operadores ou uma pipeline;
- `options` muda a execução, não os fields do documento.
...
No update document, `$set` atribui os pares existentes em `changes` e `$currentDate` pede ao servidor que calcule `updatedAt`. Nas options:

- `upsert: false` impede insert quando não existe match;
- `arrayFilters` define quais elementos correspondem a identificadores como `$[item]` usados no update;
- `hint` escolhe um índice para localizar matches;
- `writeConcern` controla acknowledgement/durabilidade.

`updateMany(filter, update, options)` tem a mesma divisão, mas aplica a transformação a todos os matches. Não se deve inferir segurança apenas por trocar `One` por `Many`.

`UpdateResult` contém `acknowledged`, `matchedCount`, `modifiedCount`, `upsertedCount` e `upsertedId`. Não contém, por defeito, os documentos finais.
...
O filtro usa `_id` para selecionar no máximo um documento. `replacement` representa o novo documento completo e não pode conter update operators de topo. `{ upsert: false }` impede criar um novo documento se esse `_id` não existir. O retorno continua a ser `UpdateResult`, não o replacement armazenado.
...
`filter` é sempre o primeiro argumento e `{ hint }` é o documento de options. `deleteOne()` elimina no máximo um match; `deleteMany()` elimina todos. `DeleteResult.deletedCount` indica quantos documentos foram eliminados. Um filtro `{}` é válido e, em `deleteMany({})`, seleciona toda a collection.
...
O segundo argumento é agora um **array**, por isso o servidor interpreta-o como update pipeline:

- o filtro seleciona documentos que possuem `subtotal`;
- `$set` é aqui um aggregation stage, não o update operator clássico, embora o nome coincida;
- `"$subtotal"` lê o valor atual do field;
- `$multiply` calcula imposto incluído e `$round` limita a duas casas;
- o output calculado substitui/cria `total` em cada documento selecionado.

Se o segundo argumento fosse `{ $set: ... }` sem array, seria um update document clássico. A forma externa determina o contexto dos operadores.
